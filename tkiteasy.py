...
class ObjetGraphique:
    """La classe ObjetGraphique sert à stocker les informations d'un dessin fait
    par tkiteazy : point, rectangle, texte, etc.
    Stocker cet objet grâce à un identifiant vous permettra ensuite de le
    supprimer ou de le modifier.

    Args:
        id (int): numéro de référence de l'objet pour Tkinter
        x (float): La position x (abscisse) de référence pour l'objet (dépend des cas)
        y (float): La position y (ordonnée) de référence pour l'objet (dépend des cas)
        couleur (str): la couleur de l'objet
...
class Position:
    """Classe servant à stocker les positions (x,y)

    Args:
        x (float): abscisse (de gauche à droite sur l'écran)
        y (float): ordonnée (de haut en bas sur l'écran)
    """
...
class Canevas(tk.Canvas):
    """C'est la classe des fenêtres graphiques. Vous ouvrirez une fenêtre et
    utiliserez ses méthodes en début de code grâce la fonction ouvrirFenetre.

    Args:
        largeur (int): la largeur (abscisse) de la fenêtre en pixels
        hauteur (int): la hauteur (ordonnée) de la fenêtre en pixels
    """
    ...
    def dessinerLigne(
        ...
        Args:
            ...
            x2 (float): La position x (abscisse) en pixels du point d'arrivée de la ligne
            y2 (float): La position y (ordonnée) en pixels du point d'arrivée de la ligne
    ...
    def afficherImage(
        ...
        return ObjetGraphique(
            self.create_image(x, y, image=photo_image, anchor=ancre),
            x,
            y,
            "",
        )
